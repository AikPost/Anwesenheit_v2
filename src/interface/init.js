...
function initKeyEventHandlers() {
    ...
    $(document).on("keydown", callbackGlobalKeypresses);

    // adds enter functionality to the courses table (go to cell below the current one on enter)
    $("#course_data_tbody").on("keypress", " input.course_data_input", callbackCourseTBodyKeypress);
}

function initSideBarButtons() {

    // 
    $("#quarter_starting_monday,#quarter_ending_monday").on("change", function () {
        $(this).val(getMondayDateString($(this).val()));
    });
...
function initCourseTableButtons() {
    ...
    $("#course_data_tbody").on("mousedown", ".move",
        function (evt) {
            console.log(evt, this);

            // only move when table row is clicked directly
            if (!["TD", "TR"].includes(evt.target.tagName)) return;
            let dragElement = $(this);
            dragElement.css("background-color", "red");
            $(".course_data_trow>td").css("cursor", "grabbing");
            ...
            $(document).on("mouseup", function () {
                ...
                $(".course_data_trow>td").css("cursor", "grab");
...
function initButtons() {
    ...
    $("#button_displayCMDRename").on("click", function () {
        $("#output_cmdRename").text(getRenameCommand());

        let range = document.createRange();
        let selection = window.getSelection();

        range.selectNodeContents($("#output_cmdRename").get(0)); // Select all the contents of the div
    ...
    $("#button_make_rename_command").on(
        "click",
        () => prompt("Folgendes Kommando in powershell ausführen", getRenameCommand())
    );
...
function initCourseTableClickHandlers() {
    $("#course_data_table > thead").on("click", "th.sortableColumn:not(.sortKeyAsc)", function () {
        $(this)
            .removeClass("sortKeyDesc")
            .addClass("sortKeyAsc")
            .siblings()
            .removeClass("sortKeyDesc sortKeyAsc");

        console.log("Sorting table", this, "by column.");
        sortInputTableByColumn($("#course_data_tbody"), $(this).index());
    });

    $("#course_data_table > thead").on("click", "th.sortableColumn.sortKeyAsc", function () {
        $(this)
            .removeClass("sortKeyAsc")
            .addClass("sortKeyDesc")
            .siblings()
            .removeClass("sortKeyDesc sortKeyAsc");
        console.log("Sorting table", this, "by column.");
        sortInputTableByColumn($("#course_data_tbody"), $(this).index(), true);
    });
}

function initTabs() {
    console.log("init tab functionality");
    function openTab(evt) {
        ...
        $(`#${$(evt.currentTarget).attr("open_tab")}`).show();
